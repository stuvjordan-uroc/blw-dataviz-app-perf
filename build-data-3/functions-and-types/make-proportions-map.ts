    ...
    .map(wave => ([
      ...
      !impVarIsIncluded(impVar, data, wave) ? null :
        new Map(vizConfig.partyGroups.map(partyGroup => ([
          partyGroup,
          {
            expanded: new Map(vizConfig.responseGroups.expanded.map((responseGroup, rgIdx, aRg) => ([
              responseGroup,
              {
            ...
            collapsed: new Map(vizConfig.responseGroups.collapsed.map((responseGroup, rgIdx, aRg) => ([
              responseGroup,
              {
                proportion: proportion(impVar, data, wave, partyGroup, responseGroup),
                prevCumProportion: rgIdx === 0 ? 0 : aRg.slice(0, rgIdx).reduce((acc, curr) =>
