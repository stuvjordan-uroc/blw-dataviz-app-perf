...
import type { Data, DataRow, ProportionGroups } from "./types.ts";
interface ResponseGroups {
  collapsed: string[][];
  expanded: string[][];
}
function weightedImpProportion(
  principle: string,
  totalWeight: number,
...
export function makeImpProportions(
  ...
  if (!data.impCols.includes(principle)) {
    console.log(
      ...
      principle,
      "to makeImpProportions, but there is no item",
      "imp_" + principle,
      "in the data."
    );
  ...
  Object.keys(responseGroups).forEach((viewType) => {
    ...
    const nonEmptyRows = data.data.filter(
      (row) =>
        responseGroups[viewType as keyof ResponseGroups]
          .flat(Infinity)
          .includes(row.imp[principle]!) && //rows that have non-empty responses on the principle
    ...
    const byResponseTotalWeight = nonEmptyRows.reduce(
      (acc, curr) => acc + curr.weight!,
      0
    );
    outProportions[viewType as keyof ResponseGroups].byResponse =
      responseGroups[viewType as keyof ResponseGroups].map((responseGroup) =>
        weightedImpProportion(
          principle,
          byResponseTotalWeight,
          responseGroup,
          nonEmptyRows
        )
      );
    ...
    outProportions[viewType as keyof ResponseGroups].byResponseAndParty =
      partyGroups.map((partyGroup) => {
        ...
        const partyGroupTotalWeight = nonEmptyRows
          ...
          .reduce((acc, curr) => acc + curr.weight!, 0);
        return responseGroups[viewType as keyof ResponseGroups].map(
          (responseGroup) =>
            weightedImpProportion(
              principle,
