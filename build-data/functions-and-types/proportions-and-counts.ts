...
import charIsIncluded from "./char-is-included.ts";
...
function proportion(
  ...
  const totalWeightResponseGroup = subset
    .filter((row) => responseGroup.includes(row[q][char] ?? ""))
...
function countsMap(
  ...
  const valuesKeyVals = [...pMap.entries()].map(
    ([rg, p]: [string[], number]) => [
      rg,
      {
  ...
  while (
    valuesKeyVals
      .map((v) => v[1])
      .reduce(
        (acc: number, curr: { rounded: number; real: number }) =>
          acc + curr.rounded,
        0
      ) < sampleSize
  ) {
    const furthestKeyValIdx: number = valuesKeyVals.reduce(
      (
        ...
        curr[1].real - curr[1].rounded >
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        array[acc]![1].real - array[acc]![1].rounded
          ? currIdx
          : acc,
      0
    );
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    valuesKeyVals[furthestKeyValIdx]![1].rounded =
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      valuesKeyVals[furthestKeyValIdx]![1].rounded + 1;
  }
  return new Map(valuesKeyVals.map(([rg, val]) => [rg, val.rounded]));
}

export default function proportionsAndCounts(
  char: string,
  q: "imp" | "perf",
  data: Data,
  vizConfig: VizConfig
        ...
        rg,
        {
          ...
          waveSplit: new Map(
            data.waves.imp.map((wave) => [
              wave,
              !charIsIncluded(char, q, data, wave)
                ? null
                : {
                    p: 0,
                    c: 0,
  ...
  pAndC.expanded.forEach(
    (
      rgvalue: PropCount & {
        ...
        waveSplit: Map<number, null | WaveSplitVal>;
      },
      rgkey: string[],
      _rgmap: Map<
        string[],
      ...
      [...rgvalue.waveSplit.entries()]
        .filter(
          ([_wave, valAtWave]: [number, null | WaveSplitVal]) =>
            valAtWave !== null
        )
        .forEach(
          (
            ...
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            valAtWave!.partySplit.forEach(
              (
                pgValue: PropCount,
                pgKey: string[],
                _pgMap: Map<string[], PropCount>
              ) => {
                pgValue.p = proportion(char, q, data, wave, pgKey, rgkey);
              }
            );
          }
        );
    ...
    (
      ...
      [...rgValue.waveSplit.entries()]
        ...
        .forEach(([_wave, valAtWave]: [number, null | WaveSplitVal]) => {
          ...
          valAtWave!.p =
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            [...valAtWave!.partySplit.values()]
              .map((valAtPg: PropCount) => valAtPg.p)
              .reduce((acc: number, curr: number) => acc + curr, 0) /
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            valAtWave!.partySplit.size;
        });
    }
  );
  //now do the parties
        ...
        (
          ...
          pgValue.p =
            [...rgValue.waveSplit.values()]
              ...
              .reduce((acc: number, curr: number) => acc + curr, 0) /
            [...rgValue.waveSplit.values()].filter(
              (waveValue: null | WaveSplitVal) => waveValue !== null
            ).length;
        }
      );
    }
    ...
    (
      ...
      rgValue.p =
        [...rgValue.waveSplit.values()]
          ...
          .reduce((acc: number, curr: number) => acc + curr, 0) /
        [...rgValue.waveSplit.values()].filter(
          (waveVal: null | WaveSplitVal) => waveVal !== null
        ).length;
    }
  );
  ...
  const countsMapsByWaveAndParty = new Map(
    data.waves.imp.map((w: number) => [
      w,
      !charIsIncluded(char, q, data, w)
        ? null
        : new Map(
            vizConfig.partyGroups.map((pg: string[]) => {
              const rgToP = new Map(
                [...pAndC.expanded.entries()].map(
                  ([rg, rgVal]: [
                    string[],
                    PropCount & {
                      waveSplit: Map<
        ...
        .forEach(([w, wVal]: [number, null | WaveSplitVal]) => {
          ...
          wVal!.partySplit.forEach(
            (
              ...
              // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
              pgVal.c = countsMapsByWaveAndParty
                .get(w)!
                .get(pgKey)!
                .get(rgKey)!;
            }
  ...
  pAndC.expanded.forEach((rgValue, _rgKey, _rgMap) => {
    rgValue.partySplit.forEach((pgValue, pgKey, _pgMap) => {
      pgValue.c = [...rgValue.waveSplit.values()]
        .filter((waveVal: null | WaveSplitVal) => waveVal !== null)
        .map((waveVal: WaveSplitVal) => waveVal.partySplit.get(pgKey))
        .reduce(
          (
            acc: number,
            curr:
