...
export default function apartmentWindows(topLeftX: number, topLeftY: number, buildingWidth: number, buildingHeight: number, numResidents: number): undefined | Resident[] {
  /* 
  ...
  */
  if (typeof numResidents !== "number" || numResidents <= 0) {
    return undefined;
  }
  if (typeof buildingHeight !== "number" || typeof buildingWidth !== "number" || buildingHeight <= 0 || buildingWidth <= 0) {
    return undefined;
  }
  ...
  const emptyWindowIndices = _.sampleSize(new Array(K ** 2).fill(1).map((el, idx) => idx), K ** 2 - numResidents);
