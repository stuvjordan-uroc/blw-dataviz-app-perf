...
export default class Data {
  ...
  utilityColumns = Object.fromEntries(this.utilityColumnNames.map(colName => ([colName, null]))) as Record<string, number | null>
  constructor(filePathString: string) {
    //read raw data from passed file path string
    const rawData = JSON.parse(strFromU8(decompressSync(fs.readFileSync(filePathString)))) as RawData
    ...
    this.utilityColumns = Object.fromEntries(
      this.utilityColumnNames
        .map(cn => (
          [cn, this.columns.findIndex(c => c === cn)]
        ))
        .map(el => ([el[0], el[1] === -1 ? null : el[1]]))
    ) as Record<string, number | null>
    //warn if any of the default column names cannot be found
    Object.keys(this.utilityColumns).forEach(ucKey => {
      if (lodash.isNull(this.utilityColumns[ucKey])) {
        console.log('WARNING: The following column is not in the raw data: ', ucKey)
      }
    })
  }
  ...
  getNonEmptyValues(columnNameStrings: string[]) {
    const colIndices = columnNameStrings.map(nameString =>
      this.columns.findIndex(col => col === nameString
      ))
    ...
    return allValues.filter(value => !this.emptyValues.includes(value)) as (string | number)[]
  }
  getImpCols() {
    const impCols = this.columns
      .map((col, idx) => ({
        colName: col,
        colIdx: idx
      }))
  ...
  setUtilityColumns(utilityColNames: string[]) {
    ...
    this.utilityColumns = Object.fromEntries(
      utilityColNames
        .map(cn => (
          [cn, this.columns.findIndex(c => c === cn)]
        )) // this is an array of arrays like this: [['weight', 42], ['noncolumn', -1]]
  ...
  sample(impVarColIndex: number, nonEmptyImpResponses: string[], wave: number, pids: string[], sampleSize: number) {
    ...
    if (waveColumnNumber && pidColumnNumber && weightColumnNumber) {
      ...
      const subset = this.data
        .filter(row => { //select rows from the requested wave
          const rowWaveValue = row[waveColumnNumber]
          return rowWaveValue && rowWaveValue === wave
        })
        .filter(row => { //select rows non-empty on requested impvar
          const rowImpValue = row[impVarColIndex] as string
          return rowImpValue && nonEmptyImpResponses.includes(rowImpValue)
        })
        .filter(row => { //select rows from the requested pid
          const rowPidResponse = row[pidColumnNumber]
          return rowPidResponse && (typeof rowPidResponse === "string") && pids.includes(rowPidResponse)
        })
        .filter(row => { //weight column index exists
      ...
      const weightsTotal = cumulativeWeights[cumulativeWeights.length - 1] as number;
      for (let sampleRowIdx = 0; sampleRowIdx < sampleSize; sampleRowIdx++) {
        const randomThreshold = Math.random() * weightsTotal
        const firstMatchIdx = cumulativeWeights.findIndex(cw => cw >= randomThreshold)
