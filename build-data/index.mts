...
const data = new Data(path.resolve('build-data', 'raw-data/dem_characteristics_importance.gz'))
...
const nonEmptyWaveValues = data.getNonEmptyValues(['wave']) as number[] | undefined
if (nonEmptyWaveValues) {
  //get imp columns
  const impColumns = data.getImpCols()
  //get nonempty imp values
  if (impColumns) {
    const nonEmptyImpResponses = data.getNonEmptyValues(impColumns.map(c => c.colName)) as string[] | undefined
    if (nonEmptyImpResponses) {
      ...
      const outSample = Object.fromEntries(impColumns.map(impCol => ([
        impCol.colName,
        Object.fromEntries(nonEmptyWaveValues.map(waveValue => ([
          "w" + waveValue.toString().padStart(2, "0"),
          []
        ])))
      ]))) as Record<string, Record<string, (string | number | null)[][]>>
      //outSample now has null (meaning an empty sample for each wave)
      ...
      impColumns.forEach(impCol => {
        nonEmptyWaveValues.forEach(waveValue => {
          ...
          const universe = data.data
            .filter(row => { //filter to wave value is waveValue
            ...
            .filter(row => { //filter to non-empty imp values
              const impValue = row[impCol.colIdx] as string | undefined
              return impValue && nonEmptyImpResponses.includes(impValue)
            })
          if (universe.length > 0) { //if there are no non-empty imp responses, leave the sample null here
            const refToImpInSample = outSample[impCol.colName]
            if (refToImpInSample) {
              const refToWaveInSample = refToImpInSample["w" + waveValue.toString().padStart(2, "0")]
              if (refToWaveInSample) {
                partyValues.forEach(arrayOfParties => {
                  refToWaveInSample.concat(
                    data.sample(impCol.colIdx, nonEmptyImpResponses, waveValue, arrayOfParties, SAMPLESIZE)
                  )
                })
              }
            }
      ...
      console.log("outsample has collections of samples for this many imp variables: ", Object.keys(outSample).length)
      ...
      nonEmptyWaveValues.forEach(waveVal => {
        ...
        if (refToImpInSample) {
          const refToWaveInSample = refToImpInSample["w" + waveVal.toString().padStart(2, "0")]
          if (refToWaveInSample) {
            console.log(`Number of rows in sample at imp_misconduct at ${"w" + waveVal.toString().padStart(2, "0")} is:`)
            console.log(refToWaveInSample.length)
          } else {
            console.log(`Something failed.  Out sample at imp_misconduct has no key ${"w" + waveVal.toString().padStart(2, "0")}`)
          }
        } else {
          console.log("Something failed.  Out sample has no top level key imp_misconduct")
        }
      })
    }
  }
