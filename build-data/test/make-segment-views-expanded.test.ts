...
import {
  ...
  byResponseAndWave,
  unSplit,
} from "../functions-and-types/make-segment-views-expanded.ts";
import fakePAndC from "./fake-pandc.ts";
import impVarIsIncluded from "../functions-and-types/char-is-included.ts";

const unsplit = unSplit(
  fakePAndC.pAndC.principle1,
  fakePAndC.layout,
  fakePAndC.data.waves.imp.length
);
describe("Unsplit segment's...", () => {
  test("count matches the pAndC count", () => {
    expect(unsplit.count).toBe(
...
describe("Map returned by byResponse...", () => {
  test.each(
    byresponse
      ...
      .map(([rg, rgVal]) => [
        rgVal.count,
        fakePAndC.pAndC.principle1.expanded.get(rg)?.c,
      ])
      .toArray()
  ...
  test("Left most edge of left most segment to be 0", () => {
    ...
    expect(
      byresponse.get(fakePAndC.vizConfig.responseGroups.expanded[0]!)
        ?.segmentCoordinates.topLeftX
    ).toBe(0);
  });
  test("Right most edge of the right most segment to be the width specified in the vizConfig", () => {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const rightMostSegment = byresponse.get(
      fakePAndC.vizConfig.responseGroups.expanded[
        fakePAndC.vizConfig.responseGroups.expanded.length - 1
      ]!
    );
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    expect(
      rightMostSegment!.segmentCoordinates.topLeftX +
        rightMostSegment!.segmentCoordinates.width
    ).toBeCloseTo(fakePAndC.layout.vizWidth);
  });
  const widthToDistribute =
    fakePAndC.layout.vizWidth - //start with the total width
    2 *
      fakePAndC.layout.pointRadius *
      fakePAndC.pAndC.principle1.expanded.size - //subtract the minimum width of each segment
    fakePAndC.layout.responseGap *
      (fakePAndC.pAndC.principle1.expanded.size - 1); //subtract the response gaps
  test.each(
    byresponse
      .entries()
      .map(([rg, rgVal]) => [
        ...
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        fakePAndC.layout.pointRadius * 2 +
          fakePAndC.pAndC.principle1.expanded.get(rg)!.p * widthToDistribute,
      ])
      .toArray()
  )(
    "Has approximately the right width for each segment",
    (segmentWidth, rightWidth) => {
  ...
  test.each(
    ...
    "Has the correct height for each segment",
    (segmentHeight, correctHeight) => {
      expect(segmentHeight).toBe(correctHeight);
    }
  );
});

const byresponseandparty = byResponseAndParty(
  fakePAndC.pAndC.principle1,
  fakePAndC.layout,
  fakePAndC.data.waves.imp.length,
...
describe("Map returned by byResponseAndParty", () => {
  //correct counts
  const tableOfCounts = byresponseandparty
    .entries()
    .map(([rg, rgVal]) => {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  test.each(tableOfCounts)(
    "has the correct count at segment for rg %p and pg %p",
    (rg, pg, segmentCount, correctCount) => {
  ...
  test("Left most edge of left most segment is 0", () => {
    ...
    const leftEdge = byresponseandparty
      ...
      .toArray()[0]!
      .values()
      .toArray()[0]!.segmentCoordinates.topLeftX;
    expect(leftEdge).toBe(0);
  });
  //right-most edge is about where it should be
  //get the last response group map
  const lastRgMap = byresponseandparty.values().toArray()[
    byresponseandparty.values().toArray().length - 1
  ];
  //get the last party group at the last responsegroup
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const lastSegment = lastRgMap!.values().toArray()[
    lastRgMap!.values().toArray().length - 1
  ];
  //get the last party group
  test("Right most edge is about where is should be", () => {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    expect(
      lastSegment!.segmentCoordinates.topLeftX +
        lastSegment!.segmentCoordinates.width
    ).toBeCloseTo(fakePAndC.layout.vizWidth);
  });
  const partyGroupTotalWidth =
    (fakePAndC.layout.vizWidth -
      fakePAndC.layout.partyGap *
        (fakePAndC.vizConfig.partyGroups.length - 1)) /
    fakePAndC.vizConfig.partyGroups.length;
  const partyGroupWidthToDistribute =
    partyGroupTotalWidth -
    2 *
      fakePAndC.layout.pointRadius *
      fakePAndC.vizConfig.responseGroups.expanded.length -
    fakePAndC.layout.responseGap *
      (fakePAndC.vizConfig.responseGroups.expanded.length - 1);
  const segmentHeight =
    fakePAndC.layout.waveHeight * fakePAndC.data.waves.imp.length;
  const tableOfSegmentWidthsAndHeights = byresponseandparty
    .entries()
    .toArray()
    .map(([rg, rgVal], rgIdx) => {
      return rgVal
        ...
        .toArray()
        .map(([pg, pgVal], pgIdx) => {
          const partyGroupTopLeftX =
            pgIdx === 0
              ? 0
              : (partyGroupTotalWidth + fakePAndC.layout.partyGap) * pgIdx;
          const rgTopLeftX =
            partyGroupTopLeftX +
            (rgIdx === 0
              ? 0
              : fakePAndC.pAndC.principle1.expanded
                  ...
                  .map(
                    (prevRgVal) =>
                      ...
                      prevRgVal.partySplit.get(pg)!.p *
                        partyGroupWidthToDistribute +
                      fakePAndC.layout.responseGap
                  )
                  .reduce((acc, curr) => acc + curr, 0));
          return [
            ...
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            2 * fakePAndC.layout.pointRadius +
              fakePAndC.pAndC.principle1.expanded.get(rg)!.partySplit.get(pg)!
                .p *
                partyGroupWidthToDistribute,
            //correct height here
            segmentHeight,
    ...
    .flat(1);
  test.each(tableOfSegmentWidthsAndHeights)(
    "has correct coordinates at %p and %p",
    (
      rg: number | string[],
      pg: number | string[],
      w: number | string[],
      ...
      tlx: number | string[],
      tly: number | string[],
      correctW: number | string[],
      correctH: number | string[],
      correctTlx: number | string[],
      correctTly: number | string[]
    ) => {
...
const byresponseandwave = byResponseAndWave(
  fakePAndC.pAndC.principle1,
  fakePAndC.layout,
  fakePAndC.data.waves.imp.length
);
describe("Map returned by byResponseAndWave", () => {
  ...
  const tableOfWavesNull = byresponseandwave
    .entries()
    .map(([rg, rgVal]) => {
      return rgVal
        .entries()
  ...
  const tableOfPointCounts = byresponseandwave
    .entries()
    .toArray()
    .map(([rg, rgVal], rgIdx) =>
      rgVal
        .entries()
        .toArray()
        .map(([wave, waveVal], waveIdx) => {
          const correctWaveVal = fakePAndC.pAndC.principle1.expanded
            .get(rg)
            ?.waveSplit.get(wave);
          return {
            rg: rg,
  ...
  const correctWidthToDistribute =
    fakePAndC.layout.vizWidth -
    2 *
      fakePAndC.layout.pointRadius *
      fakePAndC.pAndC.principle1.expanded.size -
    fakePAndC.layout.responseGap *
      (fakePAndC.pAndC.principle1.expanded.size - 1);
  const tableOfCoordinates = byresponseandwave
    .entries()
    .toArray()
    .map(([rg, rgVal], rgIdx) => {
      return rgVal
        ...
        .map(([wave, waveVal], waveIdx) => {
          ...
          const correctTopLeftX = fakePAndC.pAndC.principle1.expanded
            .values()
            .take(rgIdx)
            .map(
              (prevRgVal) =>
            ...
            .reduce((acc, curr) => acc + curr, 0);
          const correctTopLeftY =
            fakePAndC.layout.labelHeight +
            (fakePAndC.layout.waveHeight + fakePAndC.layout.labelHeight) *
              waveIdx;
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          const correctWidth =
            2 * fakePAndC.layout.pointRadius +
            fakePAndC.pAndC.principle1.expanded.get(rg)!.waveSplit.get(wave)!
              .p *
              correctWidthToDistribute;
...
const byresponseandpartyandwave = byResponseAndPartyAndWave(
  fakePAndC.pAndC.principle1,
  fakePAndC.layout,
  fakePAndC.data.waves.imp,
  fakePAndC.vizConfig.partyGroups.length
);
describe("Map returned byResponseAndPartyAndWave", () => {
  //test that it's null at the waves where it should be null
  const tableOfIsNulls = byresponseandpartyandwave
    .entries()
    .toArray()
    .map(([rg, rgVal]) =>
      rgVal
  ...
  const tableOfCounts = byresponseandpartyandwave
    .entries()
    .toArray()
    .map(([rg, rgVal]) =>
      rgVal
          ...
          waveVal!
            ...
            .map(([pg, pgVal]) => {
              return {
                ...
                count: pgVal.count,
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                correctCount: fakePAndC.pAndC.principle1.expanded
                  .get(rg)!
                  .waveSplit.get(wave)!
                  .partySplit.get(pg)!.c,
              };
  ...
  const tableOfCoordinates = byresponseandpartyandwave
    .entries()
    .toArray()
    .map(([rg, rgVal], rgIdx) =>
      rgVal
              ...
              const partyGroupTotalWidth =
                (fakePAndC.layout.vizWidth -
                  fakePAndC.layout.partyGap *
                    (fakePAndC.vizConfig.partyGroups.length - 1)) /
                fakePAndC.vizConfig.partyGroups.length;
              const partyGroupWidthToBeDistributed =
                partyGroupTotalWidth -
                2 *
                  fakePAndC.layout.pointRadius *
                  fakePAndC.pAndC.principle1.expanded.size -
                ...
                fakePAndC.pAndC.principle1.expanded
                  ...
                  .map(
                    (prevRgVal) =>
                      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                      2 * fakePAndC.layout.pointRadius +
                      prevRgVal.waveSplit.get(wave)!.partySplit.get(pg)!.p *
                        partyGroupWidthToBeDistributed +
                      fakePAndC.layout.responseGap
                  )
                  .reduce((acc, curr) => acc + curr, 0);
              const waveTopLeftY =
                fakePAndC.layout.labelHeight +
                (fakePAndC.layout.waveHeight + fakePAndC.layout.labelHeight) *
                  fakePAndC.data.waves.imp.findIndex((w) => w === wave);
              // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
              const width =
                2 * fakePAndC.layout.pointRadius +
                fakePAndC.pAndC.principle1.expanded
