...
export default function standaloneSVG(
  ...
  const svgDoc = new DOMImplementation().createDocument(SVG_NAMESPACE_URI, 'svg', null)
  //get the root element
  const svgRoot = svgDoc.documentElement
  //set the svg attributes
  svgRoot.setAttribute('width', (2 * pointRadius).toString())
  svgRoot.setAttribute('height', (2 * pointRadius).toString())
  ...
  for (const attr of [["strokeWidth", "stroke-width"], ["strokeOpacity", "stroke-opacity"], ["fillOpacity", "fill-opacity"]]) {
    circle.setAttribute(attr[1]!, circleConfig[attr[0] as keyof CircleConfig].toString())
  }
  //fill
